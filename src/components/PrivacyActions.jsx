...
const PrivacyActions = ({
  ...
  const [showContactSelectionModal, setShowContactSelectionModal] =
    useState(false);
    ...
    const resolveMyLexie = async () => {
      try {
        ...
        const resp = await fetch(
          `/api/wallet-metadata?action=by-wallet&railgunAddress=${encodeURIComponent(
            railgunAddress
          )}`
        );
        if (!resp.ok) {
  ...
  const availableTokens = useMemo(() => {
    ...
    if (activeTab === "shield") {
      ...
      return [
        ...
        {
          symbol: "USDC",
          address: "0xA0b86a33E6441c0086ec7a4dC2c7c37C1A5e01b4",
          name: "USD Coin",
        },
        {
          symbol: "USDT",
  ...
  useEffect(() => {
    ...
    setSelectedToken((prev) => {
      ...
      const mapped = prev
        ? availableTokens.find((t) => areTokensEqual(t, prev))
        : null;
  ...
  const resetFormState = useCallback(() => {
    ...
    console.log(
      "[PrivacyActions] 🔄 Before reset - isProcessing:",
      isProcessing,
      "selectedToken:",
    ...
    setSelectedToken((prev) => {
      if (!Array.isArray(availableTokens) || availableTokens.length === 0) {
        console.log(
          "[PrivacyActions] 🔄 No available tokens, setting selectedToken to null"
        );
        return null;
      }
      ...
      console.log(
        "[PrivacyActions] 🔄 Reset selectedToken - prev:",
        prev?.symbol,
        "mapped:",
        mapped?.symbol,
        "new:",
      ...
      console.log(
        "[PrivacyActions] 🔄 Token shapes - prev addr:",
        getTokenAddress(prev),
        "new addr:",
        getTokenAddress(newToken)
      );
  ...
  useEffect(() => {
    ...
    setSelectedToken((prev) => {
      ...
      if (prev) {
        const stillAvailable = availableTokens.find((t) =>
          areTokensEqual(t, prev)
        );
  ...
  const handleShieldTransactionDropped = useCallback((event) => {
    console.log(
      "[PrivacyActions] 🚫 Shield transaction dropped - unlocking UI:",
      event.detail
    );
  ...
  useEffect(() => {
    const handleBalanceUpdateComplete = (event) => {
      ...
      if (activeTransactionMonitors === 0) {
        console.log(
          "[PrivacyActions] 🔓 Balance update completed (backup unlock) - no active monitors"
        );
        ...
        console.log(
          "[PrivacyActions] 🔒 Balance update received but transaction monitors still active - keeping UI locked"
        );
      }
    };
    ...
    const handleTransactionMonitorComplete = (event) => {
      ...
      console.log(
        `[PrivacyActions] ✅ Transaction monitor completed for ${transactionType} (${
          found ? "found" : "timeout"
        }) in ${elapsedTime / 1000}s`
      );

      setActiveTransactionMonitors((prev) => {
        const newCount = prev - 1;
        console.log(
          `[PrivacyActions] 📊 Local monitor count decreased: ${newCount}`
        );

        // Only unlock UI when ALL monitors have completed
        if (newCount === 0) {
          console.log(
            "[PrivacyActions] 🔓 All local monitors completed, unlocking transaction actions"
          );
          setIsProcessing(false); // Now we can stop showing "Processing..."
          setIsTransactionLocked(false); // And unlock the UI
        } else {
          console.log(
            `[PrivacyActions] 🔒 Still ${newCount} local monitor(s) running, keeping actions locked`
          );
        }
      ...
      if (
        ...
        console.log(
          `[PrivacyActions] 🎯 Transaction monitor completed successfully for ${transactionType}, triggering points update`
        );
    ...
    const handleAbortAllRequests = () => {
      console.log(
        "[PrivacyActions] 🛑 Received abort-all-requests event - cancelling all ongoing processes"
      );
    ...
    if (typeof window !== "undefined") {
      window.addEventListener(
        "railgun-public-refresh",
        handleBalanceUpdateComplete
      );
      window.addEventListener(
        "transaction-monitor-complete",
        handleTransactionMonitorComplete
      );
      window.addEventListener("abort-all-requests", handleAbortAllRequests);
      window.addEventListener(
        "shield-transaction-dropped",
  ...
  const exceedsAvailableBalance = useMemo(() => {
    ...
    try {
      ...
      const roundedBalanceInWei = roundBalanceTo8Decimals(
      ...
      console.warn(
        "[Balance Validation] Error in exceedsAvailableBalance check:",
        error
      );
    ...
    const runGasEstimation = async () => {
      try {
        ...
        const transactionType =
          activeTab === "transfer" ? "transfer" : "unshield";
  ...
  const feeInfo = useMemo(() => {
    ...
    const usdValue = selectedToken.balanceUSD
      ? typeof selectedToken.balanceUSD === "string" &&
        selectedToken.balanceUSD.startsWith("$")
    ...
    const amountUSD = hasValidUSD
      ? usdValue * (numAmount / selectedToken.numericBalance)
      : 0;
    ...
    if (hasValidUSD) {
      ...
      const gasFeeInTokens =
        gasFeeData && selectedToken.decimals
          ? parseFloat(gasFeeData.gasCostNative || "0")
          : 0;
  ...
  const calculateMaxAmount = useCallback(() => {
    ...
    try {
      const preciseDecimal = ethers.formatUnits(
        selectedToken.balance || "0",
        selectedToken.decimals
      );
      ...
      console.warn(
        "[Max Button] Failed to calculate precise amount, falling back to numericBalance:",
        error
      );
  ...
  const monitorShieldTransaction = useCallback(
    async (transactionHash, chainId, provider) => {
      ...
      const checkTransaction = async () => {
        try {
            ...
            if (receipt.status === 0) {
              ...
              toast.custom(
                (t) => (
                  ...
                  >
                    <div className="text-red-200 border rounded-lg shadow-2xl border-red-500/30 bg-black/90">
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <button
                          ...
                          className="flex items-center justify-center w-5 h-5 ml-2 rounded hover:bg-red-900/30 text-red-300/80"
                        >
                          ×
                        </button>
                      </div>
          ...
          if (Date.now() - startTime > maxWaitTime) {
            console.warn(
              "[ShieldMonitor] Transaction monitoring timeout reached - letting PrivacyActions handle assumed success"
            );
  ...
  const shouldShowMemo = useMemo(() => {
    return (
      activeTab === "transfer" &&
      (recipientType === "railgun" || recipientType === "lexie")
    );
  ...
  const getEncryptionKey = useCallback(async () => {
    try {
      ...
      console.log(
        "[PrivacyActions] Getting signature from Redis to match WalletContext..."
      );
      ...
      console.log(
        "[PrivacyActions] Using signature from Redis (matches WalletContext)"
      );
      ...
      if (error.code === 4001 || error.message.includes("rejected")) {
        throw new Error(
          "Signature required for vault operations. Please approve the signature request."
        );
      }
  ...
  const handleShield = useCallback(async () => {
    ...
    try {
      ...
      const amountInUnits = parseTokenAmount(
        actualAmount,
        selectedToken.decimals
      );
      ...
      toastId = toast.custom(
        (t) => (
          ...
          >
            <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <button
                  ...
                  className="flex items-center justify-center w-5 h-5 ml-2 rounded cursor-pointer hover:bg-green-900/30 text-green-300/80"
                >
                  ×
                </button>
              </div>
      ...
      const isNativeToken =
        !tokenAddr && nativeTokenSymbols.includes(selectedToken.symbol);
      if (!tokenAddr && !isNativeToken) {
        console.error(
          "[PrivacyActions] Shield failed: Invalid token address",
          selectedToken
        );
              ...
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div>
                  <div className="text-sm">
                    Adding {amount} {selectedToken.symbol} to your vault...
                  </div>
                </div>
                <button
                  type="button"
      ...
      console.log(
        "[PrivacyActions] Sending shield transaction:",
        result.transaction
      );
      ...
      const txForSending = {
        ...result.transaction,
        gasLimit: result.paddedGasEstimate
          ? "0x" + BigInt(result.paddedGasEstimate).toString(16)
          : undefined, // Use padded gas limit for safety
        gasPrice: result.transaction.gasPrice
          ? "0x" + result.transaction.gasPrice.toString(16)
          : undefined,
        maxFeePerGas: result.transaction.maxFeePerGas
          ? "0x" + result.transaction.maxFeePerGas.toString(16)
          : undefined,
        maxPriorityFeePerGas: result.transaction.maxPriorityFeePerGas
          ? "0x" + result.transaction.maxPriorityFeePerGas.toString(16)
          : undefined,
        value: result.transaction.value
          ? "0x" + result.transaction.value.toString(16)
          : "0x0",
      };

      console.log(
        "[PrivacyActions] Formatted transaction for sending:",
        txForSending
      );
          ...
          >
            <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div>
                  <div className="text-sm">
                    Adding {actualAmount} {selectedToken.symbol} to your vault
                  </div>
      ...
      try {
        ...
        const { monitorTransactionInGraph } = await import(
          "../utils/railgun/transactionMonitor.js"
        );
        ...
        monitorTransactionInGraph({
          ...
          .then(async (result) => {
            if (result.found) {
              console.log(
                `[PrivacyActions] Shield monitoring completed in ${
              ...
              try {
                console.log(
                  "[PrivacyActions] 🎯 Checking if points need to be awarded..."
                );

                // First resolve Lexie ID from Railgun address
                const lexieResponse = await fetch(
                  "/api/wallet-metadata?action=by-wallet&railgunAddress=" +
                    encodeURIComponent(railgunAddress)
                );
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for points award"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for points award"
                  );
                  return;
                }
                ...
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for points award:",
                  lexieId
                );

                // Calculate actual USD value for points
                const amountInUnitsForPoints = parseTokenAmount(
                  actualAmount,
                  selectedToken.decimals
                );
                ...
                const convertTokenAmountToUSD =
                  transactionMonitor.default.convertTokenAmountToUSD;
                const usdValue = await convertTokenAmountToUSD(
                  amountInUnitsForPoints,
                  tokenAddr,
                  chainId
                );

                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for points fallback:",
                  {
                    amount: actualAmount,
                ...
                const pointsResponse = await fetch(
                  "/api/wallet-metadata?action=rewards-award",
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded via fallback (10x multiplier):",
                      {
                  ...
                  console.warn(
                    "[PrivacyActions] Points award failed:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Points fallback failed:",
                  pointsError
                );
              }
            } else {
              console.warn(
                "[PrivacyActions] Shield monitoring timed out after 30s - assuming success and proceeding"
              );

              // 🎯 TIMEOUT SUCCESS: Treat timeout as assumed success - run all same logic as confirmed Graph success
              try {
                console.log(
                  "[PrivacyActions] 🎯 Processing assumed success after timeout..."
                );
                ...
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for assumed success points"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for assumed success points"
                  );
                  return;
                }
                ...
                const lexieId = lexieData.lexieID.toLowerCase();
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for assumed success:",
                  lexieId
                );
                ...
                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for assumed success:",
                  {
                    amount: actualAmount,
                    amountInUnits: amountInUnitsForPoints,
                    tokenAddress: tokenAddr,
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded for assumed success (timeout):",
                      {
                        awarded: pointsData.awarded,
                        balance: pointsData.balance,
                        multiplier: pointsData.multiplier,
                  ...
                  console.warn(
                    "[PrivacyActions] Points award failed for assumed success:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Points processing failed for assumed success:",
                  pointsError
                );
              }
              ...
              try {
                const { syncBalancesAfterTransaction } = await import(
                  "../utils/railgun/syncBalances.js"
                );
                ...
                console.log(
                  "[PrivacyActions] ✅ Balance refresh triggered for assumed success"
                );
              } catch (balanceError) {
                console.warn(
                  "[PrivacyActions] ⚠️ Balance refresh failed for assumed success:",
                  ...
                  >
                    <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div>
                          <div className="text-sm">
                            ✅ Shielded {amount} {selectedToken.symbol}
                          </div>
                          <div className="text-xs text-green-400/80">
              ...
              if (typeof window !== "undefined") {
                window.dispatchEvent(
                  new CustomEvent("transaction-monitor-complete", {
                    detail: {
                      transactionType: "shield",
                      found: false,
                      elapsedTime: 30000,
          ...
          .catch((error) => {
            console.error(
              "[PrivacyActions] Shield Graph monitoring failed:",
              error
            );
        ...
        console.error(
          "[PrivacyActions] Failed to start shield monitoring:",
          monitorError
        );
      ...
      console.error(
        "[PrivacyActions] Shield operation failed: Network error. Please retry your transaction"
      );
      toast.dismiss(toastId);
            ...
            >
              <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div>
                    <div className="text-sm">
                      Failed to add {amount} {selectedToken?.symbol || "token"}{" "}
                      to your vault
                    </div>
                    <div className="text-xs text-green-400/80">
                      Network error. Please retry your transaction
                    </div>
                  </div>
                </div>
              </div>
  ...
  const handleUnshield = useCallback(async () => {
    ...
    if (!tokenAddr && !isNativeToken) {
      console.error(
        "[PrivacyActions] Unshield failed: Invalid token address",
        selectedToken
      );
      toast.error("Selected token is invalid. Please reselect the token.");
      return;
    ...
    try {
      ...
      const toAddress =
        activeTab === "unshield" ? address : recipientAddress || address;
      ...
      console.log("[PrivacyActions] Starting unshield operation:", {
        ...
        validationStatus: {
          ...
          tokenAddressValid:
            tokenAddr?.startsWith("0x") && tokenAddr.length === 42,
        },
      });
      ...
      console.log("[PrivacyActions] 🔍 Unshield parameters validation:", {
        ...
        tokenAddressValid:
          unshieldParams.tokenAddress?.startsWith("0x") &&
          unshieldParams.tokenAddress.length === 42,
        ...
        hasDecimals:
          unshieldParams.decimals !== undefined &&
          unshieldParams.decimals !== null, // 🚨 CRITICAL
        decimalsValue: unshieldParams.decimals, // Show actual decimals value
        isUSDT: selectedToken.symbol === "USDT",
        isCorrectUSDTDecimals:
          selectedToken.symbol === "USDT" &&
          (unshieldParams.decimals === 6 ||
            (chainId === 56 && unshieldParams.decimals === 18)),
      ...
      try {
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("railgun-optimistic-unshield", {
              detail: {
                tokenSymbol: selectedToken.symbol,
                tokenAddress: tokenAddr,
      ...
      console.log(
        "[PrivacyActions] Starting Graph-based unshield monitoring..."
      );
      ...
      try {
        ...
        monitorTransactionInGraph({
          ...
          listener: async (event) => {
            console.log(
              `[PrivacyActions] ✅ Unshield tx ${result.transactionHash} indexed on chain ${chainConfig.id}`
            );
          ...
          .then(async (monitorResult) => {
            if (monitorResult.found) {
              console.log(
                `[PrivacyActions] Unshield monitoring completed in ${
                  monitorResult.elapsedTime / 1000
                }s`
              );
              ...
              try {
                console.log(
                  "[PrivacyActions] 🎯 Checking if points need to be awarded for unshield..."
                );
                ...
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for unshield points award"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for unshield points award"
                  );
                  return;
                }
                ...
                const lexieId = lexieData.lexieID.toLowerCase();
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for unshield points award:",
                  lexieId
                );
                ...
                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for unshield points fallback:",
                  {
                    amount: actualAmount,
                    amountInUnits: amountInUnitsForPoints,
                    tokenAddress: tokenAddr,
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded for unshield via fallback (10x multiplier):",
                      {
                        awarded: pointsData.awarded,
                        balance: pointsData.balance,
                        multiplier: pointsData.multiplier,
                  ...
                  console.warn(
                    "[PrivacyActions] Unshield points award failed:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Unshield points fallback failed:",
                  pointsError
                );
              }
            } else {
              console.warn(
                "[PrivacyActions] Unshield monitoring timed out after 30s - assuming success and proceeding"
              );

              // 🎯 TIMEOUT SUCCESS: Treat timeout as assumed success - run all same logic as confirmed Graph success
              try {
                console.log(
                  "[PrivacyActions] 🎯 Processing assumed success for unshield after timeout..."
                );
                ...
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for assumed unshield success points"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for assumed unshield success points"
                  );
                  return;
                }
                ...
                const lexieId = lexieData.lexieID.toLowerCase();
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for assumed unshield success:",
                  lexieId
                );
                ...
                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for assumed unshield success:",
                  {
                    amount: actualAmount,
                    amountInUnits: amountInUnitsForPoints,
                    tokenAddress: tokenAddr,
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded for assumed unshield success (timeout):",
                      {
                        awarded: pointsData.awarded,
                        balance: pointsData.balance,
                        multiplier: pointsData.multiplier,
                  ...
                  console.warn(
                    "[PrivacyActions] Points award failed for assumed unshield success:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Points processing failed for assumed unshield success:",
                  pointsError
                );
              }
              ...
              try {
                ...
                console.log(
                  "[PrivacyActions] ✅ Balance refresh triggered for assumed unshield success"
                );
              } catch (balanceError) {
                console.warn(
                  "[PrivacyActions] ⚠️ Balance refresh failed for assumed unshield success:",
                  balanceError?.message
                );
              }
              ...
              if (typeof window !== "undefined") {
                window.dispatchEvent(
                  new CustomEvent("railgun-public-refresh", {
                    detail: { chainId },
                  })
                );
              }
                  ...
                  >
                    <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div>
                          <div className="text-sm">
                            ✅ Unshielded {amount} {selectedToken.symbol}
                          </div>
                          <div className="text-xs text-green-400/80">
                            Transaction confirmed - balances updated
                          </div>
            ...
            if (typeof window !== "undefined") {
              window.dispatchEvent(
                new CustomEvent("transaction-monitor-complete", {
                  detail: {
                    transactionType: "unshield",
                    found: monitorResult.found,
                    elapsedTime: monitorResult.elapsedTime,
                  },
                })
              );
            }
          ...
          .catch((error) => {
            console.error(
              "[PrivacyActions] Unshield Graph monitoring failed:",
              error
            );
            // Dispatch transaction monitor completion event even on failure
            if (typeof window !== "undefined") {
        ...
        console.error(
          "[PrivacyActions] Failed to start unshield monitoring:",
          error
        );
        // Dispatch transaction monitor completion event even if failed to start
        if (typeof window !== "undefined") {
      ...
      if (
        ...
        error.message.includes(
          "Cannot calculate gas reclamation: no price available for fee token"
        )
      ) {
              ...
              <div className="text-red-200 border rounded-lg shadow-2xl border-red-500/30 bg-black/90">
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-xs text-red-400/80">
                      Balance is too low to pay for the fees. Please increase
      ...
      if (
        error.message &&
        error.message.includes("You don't have enough funds to cover the fees")
      ) {
              ...
              <div className="text-red-200 border rounded-lg shadow-2xl border-red-500/30 bg-black/90">
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div>
                    <div className="text-sm font-bold">TRANSACTION FAILED</div>
                    <div className="mt-1 text-xs text-red-400/80">
                      Insufficient funds to cover transaction fees. Please try a
      ...
      // Check for specific SnarkJS proof generation failure
        ...
        error.message &&
        error.message.includes("SnarkJS failed to fullProveRailgun")
      ) {
        toast.custom(
          (t) => (
            <div
              ...
              <div className="text-red-200 border rounded-lg shadow-2xl border-red-500/30 bg-black/90">
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div>
                    <div className="text-sm font-bold">TRANSACTION FAILED</div>
                    <div className="mt-1 text-xs text-red-400/80">
                      Max amount exceeds available vault balance. Please try
                      again with a slightly lower amount.
                    </div>
                  </div>
                  <button
                    type="button"
  ...
  const handleTransfer = useCallback(async () => {
    if (
      ...
      !amount ||
      !isValidAmount ||
      !isValidRailgunAddress(railgunAddress)
    ) {
      return;
    }
    ...
    if (!isValidRailgunAddress(recipientAddress)) {
      ...
      try {
        const resp = await fetch(
          `/api/wallet-metadata?action=lexie-resolve&lexieID=${encodeURIComponent(
            input
          )}`
        );
        if (!resp.ok) {
          ...
          toast.custom(
            (t) => (
              ...
              >
                <div className="text-green-200 border rounded-lg shadow-2xl border-red-500/30 bg-black/90">
                  <div className="flex items-center gap-3 px-4 py-3">
                    <div className="w-3 h-3 bg-red-400 rounded-full" />
                    <div>
                      <div className="text-sm">
                        LexieID does not exist or is not linked to a LexieVault
                      </div>
                    </div>
                  </div>
                </div>
    ...
    try {
          ...
          <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <div>
                ...
                <div className="text-xs text-green-400/80">
                  Encrypting and preparing proofs
                </div>
              </div>
              <button
                type="button"
      ...
      if (!tokenAddr && !isNativeToken) {
        console.error(
          "[PrivacyActions] Transfer failed: Invalid token address",
          selectedToken
        );
        toast.error("Selected token is invalid. Please reselect the token.");
        return;
      ...
      const timelineRecipientAddress =
        tx.resolvedRecipientAddress || recipientAddress;
      ...
      setTimeout(() => {
        console.log(
          "🔄 [PrivacyActions] Triggering transaction history refresh after transfer"
        );
      ...
      try {
        ...
        console.log(
          "[PrivacyActions] 🚀 Starting transfer transaction monitoring:",
          {
        ...
        monitorTransactionInGraph({
          ...
          .then(async (result) => {
            if (result.found) {
              console.log(
                `[PrivacyActions] Transfer monitoring completed in ${
                  result.elapsedTime / 1000
                }s`
              );
              ...
              try {
                console.log(
                  "[PrivacyActions] 🎯 Checking if points need to be awarded for transfer..."
                );
                ...
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for transfer points award"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for transfer points award"
                  );
                  return;
                }
                ...
                const lexieId = lexieData.lexieID.toLowerCase();
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for transfer points award:",
                  lexieId
                );
                ...
                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for transfer points fallback:",
                  {
                    amount: actualAmount,
                    amountInUnits: amountInUnitsForPoints,
                    tokenAddress: tokenAddr,
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded for transfer via fallback (10x multiplier):",
                      {
                        awarded: pointsData.awarded,
                        balance: pointsData.balance,
                        multiplier: pointsData.multiplier,
                  ...
                  console.warn(
                    "[PrivacyActions] Transfer points award failed:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Transfer points fallback failed:",
                  pointsError
                );
              }
            } else {
              console.warn(
                "[PrivacyActions] Transfer monitoring timed out after 30s - assuming success and proceeding"
              );

              // 🎯 TIMEOUT SUCCESS: Treat timeout as assumed success - run all same logic as confirmed Graph success
              try {
                console.log(
                  "[PrivacyActions] 🎯 Processing assumed success for transfer after timeout..."
                );
                ...
                if (!lexieResponse.ok) {
                  console.warn(
                    "[PrivacyActions] Could not resolve Lexie ID for assumed transfer success points"
                  );
                  return;
                }
                ...
                if (!lexieData?.success || !lexieData?.lexieID) {
                  console.warn(
                    "[PrivacyActions] No Lexie ID found for assumed transfer success points"
                  );
                  return;
                }
                ...
                const lexieId = lexieData.lexieID.toLowerCase();
                console.log(
                  "[PrivacyActions] ✅ Resolved Lexie ID for assumed transfer success:",
                  lexieId
                );
                ...
                console.log(
                  "[PrivacyActions] 💰 Calculated USD value for assumed transfer success:",
                  {
                    amount: actualAmount,
                    amountInUnits: amountInUnitsForPoints,
                    tokenAddress: tokenAddr,
                ...
                if (pointsResponse.ok) {
                  ...
                  if (pointsData?.success) {
                    console.log(
                      "[PrivacyActions] ✅ Points awarded for assumed transfer success (timeout):",
                      {
                        awarded: pointsData.awarded,
                        balance: pointsData.balance,
                        multiplier: pointsData.multiplier,
                  ...
                  console.warn(
                    "[PrivacyActions] Points award failed for assumed transfer success:",
                    await pointsResponse.text()
                  );
                }
              } catch (pointsError) {
                console.warn(
                  "[PrivacyActions] Points processing failed for assumed transfer success:",
                  pointsError
                );
              }
              ...
              try {
                ...
                console.log(
                  "[PrivacyActions] ✅ Balance refresh triggered for assumed transfer success"
                );
              } catch (balanceError) {
                console.warn(
                  "[PrivacyActions] ⚠️ Balance refresh failed for assumed transfer success:",
                  balanceError?.message
                );
              }
                  ...
                  >
                    <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div>
                          <div className="text-sm">
                            ✅ Transferred {amount} {selectedToken.symbol}
                          </div>
                          <div className="text-xs text-green-400/80">
                            Transaction confirmed - balances updated
                          </div>
          ...
          .catch((error) => {
            console.error(
              "[PrivacyActions] Transfer Graph monitoring failed:",
              error
            );
            // Dispatch transaction monitor completion event even on failure
            if (typeof window !== "undefined") {
        ...
        console.error(
          "[PrivacyActions] Failed to start transfer monitoring:",
          error
        );
        // Dispatch transaction monitor completion event even if failed to start
        if (typeof window !== "undefined") {
  ...
  if (isLoading) {
    return (
      <div className="p-6 border rounded bg-black/40 border-green-500/20">
        <div className="animate-pulse">
          <div className="h-6 mb-4 rounded bg-green-500/20"></div>
          <div className="h-32 rounded bg-green-500/20"></div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 border rounded bg-black/40 border-green-500/20">
        <div className="py-8 text-center">
          <EyeSlashIcon className="w-12 h-12 mx-auto mb-4 text-green-400/70" />
          <h3 className="mb-2 text-lg font-medium text-emerald-300">
            Connect Your Wallet
          </h3>
  ...
  if (!isChainSupported) {
    return (
      <div className="p-6 border rounded bg-black/40 border-green-500/20">
        <div className="py-8 text-center">
          <ExclamationTriangleIcon className="w-12 h-12 mx-auto mb-4 text-yellow-300" />
          <h3 className="mb-2 text-lg font-medium text-emerald-300">
            Unsupported Network
          </h3>
  ...
  if (!canUseRailgun) {
    return (
      <div className="p-6 border rounded bg-black/40 border-green-500/20">
        <div className="py-8 text-center">
          <div className="w-12 h-12 mx-auto mb-4 border-b-2 rounded-full animate-spin border-emerald-400"></div>
          <h3 className="mb-2 text-lg font-medium text-emerald-300">
            Initializing Vault
          </h3>
  ...
  return (
    <div className="overflow-hidden border rounded shadow-lg bg-black/40 border-green-500/20">
      {/* Header */}
      ...
      <div className="px-6 py-3 border-b border-green-500/20">
            ...
            return (
              <>
                ...
                <span className="font-medium">
                  {currentTab?.name || "Action"}
                </span>
                <span className="text-sm break-words text-green-400/70">
        ...
        <div className="px-6 py-4 border-b border-green-500/20">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-emerald-300">
              {getCurrentNetwork()?.name || "Network"} Vault Balances
            </h3>
          </div>
            ...
            {isPrivateBalancesLoading ? (
              ...
              <div className="space-y-2">
                <div className="text-sm text-green-400/70">
                  {privateBalances.length} Vault Token
                  {privateBalances.length !== 1 ? "s" : ""}
                </div>
                {privateBalances.map((token) => (
                  <div
                    key={getTokenAddress(token) || token.symbol}
                    className="p-2 text-sm border rounded bg-black/60 border-green-500/10"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    {token.balanceUSD !== undefined && (
                      <div className="mt-1 text-right text-green-400/70">
                        $
                        {typeof token.balanceUSD === "string" &&
                        token.balanceUSD.startsWith("$")
                          ? token.balanceUSD.substring(1)
                          : token.balanceUSD}
                      </div>
                    )}
                  </div>
              ...
              <div className="text-sm text-green-400/70">
                No vault tokens yet
                <br />
                Add some tokens to start using secure vault
              </div>
            )}
          </div>
        </div>
      ...
      <div className="p-6 text-green-300">
        {activeTab === "receive" ? (
          ...
          <div className="space-y-6">
            ...
            {paymentLink && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-center text-green-400/70">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  <input
                    ...
                    value={paymentLink}
                    readOnly
                    className="flex-1 w-full min-w-0 px-3 py-2 text-sm text-green-200 bg-black border rounded border-green-500/40"
                  />
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex items-center justify-center w-full gap-1 px-3 py-2 border rounded sm:w-auto bg-emerald-600/30 hover:bg-emerald-600/50 text-emerald-200 border-emerald-400/40"
                  >
            ...
            {/* Instructions */}
            <div className="p-4 border rounded bg-black/60 border-green-500/20">
          ...
          <div className="space-y-6">
            ...
            <div className="text-center">
              <UsersIcon className="w-12 h-12 mx-auto mb-3 text-green-300" />
              <h3 className="mb-1 text-lg font-bold text-green-300">
                Contacts Manager
              </h3>
              <p className="text-sm text-green-400/70">
                Save frequently used addresses for easy sending
              </p>
            </div>
            ...
            <button
              ...
              className="flex items-center justify-center w-full gap-2 px-4 py-3 font-medium transition-colors border rounded bg-emerald-600/30 hover:bg-emerald-600/50 text-emerald-200 border-emerald-400/40"
            >
            ...
            {contacts.length === 0 ? (
              <div className="py-8 text-center">
                <UsersIcon className="w-16 h-16 mx-auto mb-4 text-green-400/30" />
                <p className="text-green-400/70">No contacts saved yet</p>
                <p className="mt-1 text-sm text-green-400/50">
              ...
              <div className="space-y-3">
                {contacts.map((contact) => (
                  <div
                    key={contact.id}
                    className="p-3 border rounded bg-black/40 border-green-500/20"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div
                          className="font-mono text-sm text-green-300 break-all transition-colors cursor-pointer select-all hover:text-green-200"
                          onClick={() => {
                            const address =
                              contact.type === "eoa"
                                ? contact.address
                      ...
                      <div className="flex gap-2 ml-3">
                        <button
                          ...
                          className="px-2 py-1 text-xs text-green-400 border rounded hover:text-green-300 border-green-500/40 hover:bg-green-900/20"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => removeContact(contact.id)}
                          className="px-2 py-1 text-xs text-red-400 border rounded hover:text-red-300 border-red-500/40 hover:bg-red-900/20"
                        >
                          Remove
                        </button>
                      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative" ref={tokenMenuRef}>
                ...
                >
                  <span>
                    {selectedToken
                      ? `${selectedToken.symbol} - ${formatBalance(
                          ...
                          selectedToken.balanceUSD !== undefined
                            ? ` ($${
                                typeof selectedToken.balanceUSD === "string" &&
                                selectedToken.balanceUSD.startsWith("$")
                                  ? selectedToken.balanceUSD.substring(1)
                ...
                {isTokenMenuOpen && (
                  <div className="absolute left-0 right-0 z-20 mt-1 overflow-auto text-green-300 bg-black border rounded shadow-xl border-green-500/40 max-h-60">
                    {availableTokens.map((token) => (
                      <button
                        ...
                        className="w-full px-3 py-2 text-left hover:bg-emerald-900/30 focus:bg-emerald-900/30 focus:outline-none"
                      >
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  step="any"
                  min="0"
                  className="w-full px-3 py-2 text-green-200 bg-black border rounded border-green-500/40"
                ...
                {selectedToken && (
                  <button
                    ...
                    className="absolute px-2 py-1 text-xs text-green-200 bg-black border rounded right-2 top-2 border-green-500/40 hover:bg-green-900/20"
                  >
                    Max
                  </button>
                )}
              ...
              {selectedToken && (
                <p className="mt-1 text-sm text-green-400/70">
                  Available: {formatBalance(selectedToken.numericBalance)}{" "}
                  {selectedToken.symbol}
                </p>
              )}
              ...
              {/* Fee Display */}
              {feeInfo && (
                <div className="p-3 mt-3 text-xs border rounded bg-black/40 border-green-500/20">
                  <div className="space-y-1">
                    ...
                    {feeInfo.gasFeeUSD && activeTab !== "shield" && (
                      <div className="flex justify-between pb-1 mb-1 border-b border-green-500/20">
                        <span className="text-green-400/80">
                          Est. Gas Fees:
                        </span>
                        <span className="text-green-200">
                          ({feeInfo.gasCostNative} {feeInfo.nativeGasToken}) $
                          {feeInfo.gasFeeUSD}
                        </span>
                      </div>
                    )}
                      ...
                      feeInfo.feeUSD !== "N/A" && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-green-200">
                            $
                            {(
                              parseFloat(feeInfo.feeUSD) +
                              parseFloat(feeInfo.gasFeeUSD)
                            ).toFixed(2)}
                          </span>
                        </div>
                      )}
                    <div className="flex justify-between font-medium">
                      ...
                      <span className="text-emerald-300">
                        ({feeInfo.netAmount} {selectedToken.symbol})
                        {feeInfo.netAmountUSD === "N/A"
                          ? ""
                          : ` $${feeInfo.netAmountUSD}`}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-1 text-xs text-green-400 transition-colors border rounded hover:text-green-300 border-green-500/40 hover:bg-green-900/20"
                            ...
                            >
                              <div className="text-green-200 border rounded-lg shadow-2xl border-green-500/30 bg-black/90">
                                <div className="flex items-center gap-3 px-4 py-3">
                                  ...
                                  <div>
                                    <div className="text-sm">
                                      Extra spaces removed from wallet address
                                    </div>
                                  </div>
                                </div>
                              </div>
                  ...
                  <div className="mt-1 text-xs text-green-400/70">
                    ...
                    {recipientType === "railgun" &&
                      "Will send to zk-shielded address (0zk...)"}
                    {recipientType === "lexie" && "Will send to LexieID"}
                    {recipientType === "invalid" &&
                      recipientAddress &&
                      "❌ Invalid address format"}
                    {recipientType === "none" &&
                      "Enter recipient address or LexieID"}
                  </div>

                  {/* Contact Suggestions Dropdown */}
                  {showSuggestions && contactSuggestions.length > 0 && (
                    <div className="absolute z-20 w-full mt-1 overflow-auto bg-black border rounded shadow-xl border-green-500/40 max-h-40">
                      {contactSuggestions.map((contact) => (
                        <button
                          ...
                          className="w-full px-3 py-2 text-left text-green-200 hover:bg-emerald-900/30 focus:bg-emerald-900/30 focus:outline-none"
                        >
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{contact.id}</span>
                            <span className="ml-2 text-xs text-green-400/70">
                              (
                              {contact.type === "lexieId"
                                ? "LexieID"
                                : "WALLET"}
                              )
                            </span>
                          </div>
                          <div className="text-xs truncate text-green-400/60">
                            {contact.type === "eoa"
                              ? contact.address
                              : contact.lexieId}
                          </div>
                  ...
                  {recipientAddress &&
                    ...
                    !contacts.some(
                      (c) =>
                        c.address === recipientAddress ||
                        c.id === recipientAddress ||
                        c.lexieId === recipientAddress
                    ) && (
                ...
                {shouldShowMemo && (
                  <div>
                    ...
                    />
                    <p className="mt-1 text-sm text-green-400/70">
                      Memo is encrypted; only sender and recipient can read it.
                    </p>
                  </div>
                )}
              </div>
            ...
            <button
              ...
              disabled={
                ...
                isTransactionLocked ||
                !selectedToken ||
                (!gasFeeData && activeTab !== "shield") ||
                (activeTab === "transfer" &&
                  (!recipientAddress || recipientType === "invalid"))
              }
            ...
            >
              {isProcessing ? (
                <div className="flex items-center justify-center gap-2">
                  <div className="w-4 h-4 border-b-2 rounded-full animate-spin border-emerald-400"></div>
                  Processing...
                </div>
              ) : isTransactionLocked ? (
                <div className="flex items-center justify-center gap-2">
                  <div className="w-4 h-4 border-b-2 border-yellow-400 rounded-full animate-spin"></div>
                  Updating Balances...
                </div>
              ) : (
                `${
        ...
        {/* Info */}
        <div className="p-4 mt-6 border rounded bg-black/60 border-green-500/20">
          <div className="flex">
      ...
      {showContactSelectionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 font-mono bg-black bg-opacity-75">
          <div className="w-full max-w-md overflow-hidden bg-gray-900 border border-gray-700 rounded-lg shadow-2xl">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
              ...
              <button
                ...
                className="transition-colors text-green-400/70 hover:text-green-300"
              >
                ✕
              </button>
            </div>
            ...
            {/* Modal Content */}
            <div className="p-6 space-y-4 overflow-y-auto text-green-300 max-h-96">
              <div>
              ...
              {contacts.length === 0 ? (
                <div className="py-8 text-center text-green-400/70">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const recipientValue =
                          contact.lexieId || contact.address;
                      ...
                      className="w-full p-3 text-left transition-colors border rounded bg-black/40 border-green-500/20 hover:bg-green-900/20 hover:border-green-500/40"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-green-200 truncate">
                            {contact.name || contact.id}
                          </div>
                          <div className="text-xs truncate text-green-400/70">
                            {contact.lexieId
                              ? `@${contact.lexieId}`
                              : contact.address}
                          </div>
                        </div>
                        <div className="ml-2 text-xs text-green-400/70">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
                <button
                  onClick={() => setShowContactSelectionModal(false)}
                  className="px-4 py-2 text-sm text-gray-300 transition-colors border border-gray-600 rounded hover:bg-gray-800"
                >
                  Cancel
                </button>
              </div>
