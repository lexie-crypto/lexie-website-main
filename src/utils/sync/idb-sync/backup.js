...
import { exportWalletSnapshot, checkVaultExists } from './exporter.js';
...
export const createWalletBackup = async (walletId, eoa) => {
  try {
    ...
    // Check if vault already exists before creating backup
    console.log('[Wallet-Backup] 🔍 Checking if vault already exists before backup...');
    const vaultExists = await checkVaultExists();

    if (vaultExists) {
      console.error('[Wallet-Backup] ❌ Vault already exists - aborting backup process');
      ...
      error.details = { vaultExists: true };
      throw error;
    }
