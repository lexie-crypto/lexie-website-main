...
/**
 * Upload wallet backup to Redis
 * @param {string} backupKey - Redis key for backup
 * @param {Object} backupData - Essential wallet data to backup
 * @returns {Promise<boolean>} Success status
 */
export const uploadWalletBackup = async (backupKey, backupData) => {
  try {
    console.log('[IDB-Backup-API] Uploading wallet backup...', {
      backupKey,
      recordCount: backupData.recordCount,
      totalBytes: backupData.totalBytes
    });

    const response = await makeSyncRequest('idb-wallet-backup-upload', {
      method: 'POST',
      body: JSON.stringify({
        backupKey,
        backupData: {
          ndjsonData: backupData.ndjsonData,
          recordCount: backupData.recordCount,
          totalBytes: backupData.totalBytes,
          timestamp: backupData.timestamp,
          walletId: backupData.walletId
        }
      })
    });

    console.log('[IDB-Backup-API] Wallet backup uploaded successfully');
    return response.success === true;

  } catch (error) {
    console.error('[IDB-Backup-API] Failed to upload wallet backup:', error);
    return false;
  }
};

/**
 * Download wallet backup from Redis
 * @param {string} backupKey - Redis key for backup
 * @returns {Promise<Object|null>} Backup data or null if not found
 */
export const downloadWalletBackup = async (backupKey) => {
  try {
    console.log('[IDB-Backup-API] Downloading wallet backup...', { backupKey });

    const response = await makeSyncRequest('idb-wallet-backup-download', {
      method: 'GET'
    }, {
      backupKey
    });

    if (!response || !response.backupData) {
      console.log('[IDB-Backup-API] No backup found');
      return null;
    }

    console.log('[IDB-Backup-API] Wallet backup downloaded successfully');
    return response.backupData;

  } catch (error) {
    console.error('[IDB-Backup-API] Failed to download wallet backup:', error);
    return null;
  }
};
