/**
 * Vercel Serverless Function - Get Private Balance Data from Redis
 * Key: private_balances:<walletId>-<chainId>
 * Returns: { updatedAt, balances: [...] }
 */
...
function initializeRedisClient() {
  try {
    ...
    console.error('[GET-PRIVATE-BALANCES] ❌ Redis client initialization failed:', error.message);
    throw error;
  }
}
...
function validateHmacSignature(req) {
  ...
  console.log('[GET-PRIVATE-BALANCES] ✅ HMAC signature validated');
}
...
export default async function handler(req, res) {
  ...
  try {
    console.log('[GET-PRIVATE-BALANCES] 🔍 Request received:', {
      method: req.method,
      url: req.url
    });
    ...
    console.log(`[GET-PRIVATE-BALANCES] 📥 Retrieving private balance data for key: ${key}`);
    ...
    try {
      // Get private balance data from Redis using proper key format
      const redisKey = `private_balances:${key}`;
      const balanceDataJson = await redisClient.get(redisKey);

      if (!balanceDataJson) {
        console.log(`[GET-PRIVATE-BALANCES] ❌ No private balance data found for key: ${key}`);
        await redisClient.quit();
        
        return res.status(404).json({
          success: false,
          error: 'Private balance data not found'
        });
      }

      const balanceData = JSON.parse(balanceDataJson);
      
      console.log(`[GET-PRIVATE-BALANCES] ✅ Found private balance data:`, {
        key,
        redisKey,
      ...
      console.error('[GET-PRIVATE-BALANCES] ❌ Redis operation failed:', redisError);
      await redisClient.quit().catch(() => {});
      
      return res.status(500).json({
        success: false,
        error: 'Failed to retrieve private balance data',
        details: redisError.message
      });
    }

  } catch (error) {
    console.error('[GET-PRIVATE-BALANCES] ❌ Handler error:', error);
