/**
 * Vercel Serverless Function - Store Balance Data in Redis
...
function initializeRedisClient() {
  try {
    ...
    console.error('[STORE-BALANCES] ❌ Redis client initialization failed:', error.message);
    throw error;
  }
}
...
function validateHmacSignature(req) {
  ...
  console.log('[STORE-BALANCES] ✅ HMAC signature validated');
}
...
export default async function handler(req, res) {
  ...
  try {
    console.log('[STORE-BALANCES] 🔍 Request received:', {
      method: req.method,
      hasBody: !!req.body
    });
    ...
    console.log(`[STORE-BALANCES] 💾 Storing balance data:`, {
    ...
    try {
      // Store balance data with proper key format
      const redisKey = `wallet_balances:${walletId}-${chainId}`;
      const balanceData = {
        updatedAt: Date.now(),
        balances: filteredBalances.map(balance => ({
          ...
          isPrivate: balance.isPrivate || false
        }))
      };
      ...
      console.log(`[STORE-BALANCES] ✅ Stored balance data in Redis:`, {
        key: redisKey,
        result,
      ...
      console.log(`[STORE-BALANCES] ✅ Verification successful`);

      await redisClient.quit();

      return res.status(200).json({
        success: true,
        message: 'Balance data stored successfully',
        stored: {
          key: redisKey,
          walletId: walletId?.slice(0, 8) + '...',
          chainId,
      ...
      console.error('[STORE-BALANCES] ❌ Redis operation failed:', redisError);
      await redisClient.quit().catch(() => {});
      
      return res.status(500).json({
        success: false,
        error: 'Failed to store balance data',
        details: redisError.message
      });
    }

  } catch (error) {
    console.error('[STORE-BALANCES] ❌ Handler error:', error);
