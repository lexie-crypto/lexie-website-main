...
export default async function handler(req, res) {
  ...
  // Handle merkletree data requests
  if (req.query.action === 'merkletree-data') {
    ...
    console.log(`🌲 [MERKLETREE-PROXY-${requestId}] ${req.method} ${subaction} for chain ${chainId}`);

    // Backend paths for merkletree operations
    const merkletreeOperations = {
      'has-data': `/api/merkletree/has-data/${chainId}`,
      'get-data': `/api/merkletree/get-data/${chainId}`,
      'store-data': `/api/merkletree/store-data/${chainId}`,
      'validate-data': `/api/merkletree/validate-data/${chainId}`
    };

    // Note: Backend routes are now at /api/merkletree/* but we're proxying through wallet-metadata
    // The backend router handles the actual routing

    const backendPath = merkletreeOperations[subaction];
    if (!backendPath) {
      return res.status(400).json({
        success: false,
        error: `Invalid subaction: ${subaction}`
      });
    }
    ...
    try {
      ...
      console.error(`❌ [MERKLETREE-PROXY-${requestId}] Error:`, error.message);
      return res.status(500).json({
        success: false,
        error: error.message.includes('timeout') ? 'Backend timeout - please try again' : 'Internal proxy error'
      });
    }
  }
